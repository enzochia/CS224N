...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = [[] for _ in range(len(partial_parses))]
    while unfinished_parses:
        minibatch = unfinished_parses[:batch_size]
        transitionList = model.predict(minibatch)
        popList = []
        for idxParse, parse in enumerate(minibatch):
            if ((len(parse.stack) == 1) and
                (len(parse.buffer) == 0)):
                popList.append(idxParse)
                dependencies[partial_parses.index(parse)] = parse.dependencies
            else:
                parse.parse_step(transitionList[idxParse])
        for idxPop in sorted(popList, reverse=True):
            unfinished_parses.pop(idxPop)
    ### END YOUR CODE
