...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        w = torch.empty(self.n_features * self.embeddings.shape[1], self.hidden_size)
        self.embed_to_hidden_weight = nn.Parameter(nn.init.xavier_uniform_(w))
        b = torch.empty(self.hidden_size)
        self.embed_to_hidden_bias = nn.Parameter(nn.init.uniform_(b))
        ...
        self.hidden_to_logits_bias = nn.Parameter(nn.init.uniform_(b))
    ...
    def embedding_lookup(self, w):
        ...
        x = self.embeddings[torch.flatten(w)].view(w.shape[0], w.shape[1] * self.embeddings.shape[1])
