...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.matmul(outsideVectors.transpose(), y_diff)
    gradOutsideVecs = np.matmul(np.matrix(centerWordVec).transpose(), np.matrix(y_diff)).transpose()
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    U_neg_samp = outsideVectors[indices, :] * np.array([1] + [-1] * K)[:, np.newaxis]
    grad_shared_term = sigmoid(np.matmul(U_neg_samp, centerWordVec)) - 1
    loss = -np.sum(np.log(grad_shared_term + 1))
    gradCenterVec = np.matmul(U_neg_samp.transpose(), grad_shared_term)
    outsideCountDiag = np.diag([-np.sum(indices == x) if idx > 0 else np.sum(indices == x)
                                for idx, x in enumerate(indices)])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[indices, :] = np.matmul(np.matmul(np.matrix(centerWordVec).transpose(),
                                                      np.matrix(grad_shared_term)),
                                            outsideCountDiag).transpose()
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        currentLoss, \
        gradCenterVec, \
        currentGradOutsideVectors = word2vecLossAndGradient(centerWordVectors[word2Ind[currentCenterWord], :],
